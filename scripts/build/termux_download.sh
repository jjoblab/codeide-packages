...
termux_download() {
        ...
        # Two User-Agents: different mirrors require different UAs.
        # - Browser UA: required by GitHub releases (returns 404 for Wget UA).
        # - Wget UA: required by Fossies (returns 401 anti-robot for browser UA
        #   that doesn't execute JavaScript).
        # Most mirrors work with either, but we try browser first (more common),
        # then Wget as fallback.
        local UA_BROWSER="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
        local UA_WGET="Wget/1.21"
        ...
        # Tries browser UA first, then Wget UA if that fails.
        ...
        _termux_download_one() {
                ...
                for ua in "$UA_BROWSER" "$UA_WGET"; do
                        curl_opts=("${CURL_OPTIONS_BASE[@]}" --user-agent "$ua")
                        if [[ "${TERMUX_QUIET_BUILD-}" == "true" ]]; then
                                curl_opts+=(--no-progress-meter)
                        fi
                        echo "Downloading ${url}"
                        if curl "${curl_opts[@]}" --output "$TMPFILE" "$url"; then
                                _termux_download_ok=1
                                return 0
                        fi
                        # If browser UA failed, try Wget UA before giving up
                        if [[ "$ua" == "$UA_BROWSER" ]]; then
                                echo "Download failed with browser UA, trying Wget UA..." 1>&2
                                continue
                        fi
                        # Both UAs failed — do the existing retry loop with Wget UA
                        local error=1
                        local retry=2
                        local delay=60
                        local try
